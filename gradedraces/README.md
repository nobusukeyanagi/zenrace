...
通知本文は「🏁M/D(曜)の公営競技」から始まり、グレードレースと競技別開催を空行で区切り、各日程ページへのURLを添えます。
...
2026年7月・8月は、競輪・オートレース・ボートレース・地方競馬・JRAの公式日程を全件突合し、次の固定監査リストへ保存しています。

```text
gradedraces/data/official_schedule_2026_07_08.json
```

監査対象は、開催日・競技・開催場・グレード・レース名です。発走時刻と優勝者は開催後も更新されるため、固定日程との同一性検査からは除外し、個別取得処理で更新します。

テストでは、2026年7月52件・8月53件について、公式リストからの欠落、余分な登録、重複を検出します。
競輪の優勝者は、公式・補完サイトから年齢や期が併記されても、公開データへ保存する際に氏名だけへ正規化します。
